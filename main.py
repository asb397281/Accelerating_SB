...
def get_expr_name(ldb=True, model='resnet', optimizer='sgd', lr=0.1,momentum=0, droprate=0, dataset='cifar10', quantization='none'):
    name = {
    ...
    quant_suffix = '' if quantization == 'none' else '-{}'.format(quantization)
    return '{}-{}-{}-{}-{}-{}{}'.format(TIME_NOW,method, dataset, model, optimizer, name, quant_suffix)


def apply_quantization(net, quantization_type, device):
    """Apply quantization to the model based on the specified type."""
    if quantization_type == 'none':
        return net
    elif quantization_type == 'float16':
        print('==> Applying float16 quantization..')
        return net.half()
    elif quantization_type == 'float8':
        print('==> Applying float8 quantization (using bfloat16 as approximation)..')
        # Float8 is not directly supported in PyTorch, using bfloat16 as alternative
        return net.to(torch.bfloat16)
    elif quantization_type == 'mixed':
        print('==> Applying mixed precision (using automatic mixed precision)..')
        # For mixed precision, we'll use the model as is but enable autocast during training
        return net
    else:
...
def build_model(args,device):
    ...
    if hasattr(args, 'quantization') and args.quantization != 'none':
        net = apply_quantization(net, args.quantization, device)
...
def train(epoch, model, device, train_loader_single_batch, optimizer, criterion, args, params,train_loader_double_batch):
    ...
    scaler = torch.cuda.amp.GradScaler() if hasattr(args, 'quantization') and args.quantization == 'mixed' and device == 'cuda' else None
...
def test(epoch,model, device, test_loader, criterion, args=None):
    model.eval()
    test_loss = 0
    test_total = 0
    test_correct = 0
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # Convert data to appropriate precision for testing
            if args and hasattr(args, 'quantization'):
                if args.quantization == 'float16':
                    data = data.half()
                elif args.quantization == 'float8':
                    data = data.to(torch.bfloat16)
...
def get_parser():
    ...
    parser.add_argument('--quantization', default='none', type=str, 
                       help='quantization type', choices=['none', 'float16', 'float8', 'mixed'])
    return parser
...
def main():    
    ...
    for epoch in range(args.epochs):
        ...
        acc=test(epoch, layerdropback_net, device, testloader, layerdropback_criterion, args)
